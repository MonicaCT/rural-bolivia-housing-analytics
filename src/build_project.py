...
def write_dictionary(households: pd.DataFrame, members: pd.DataFrame) -> None:
    descriptions = {
        ...
        "vulnerability_index": "Analytically defined index from housing, education and income (0-100)",
        "survey_weight": "Synthetic survey weight used for methodological evaluation",
    }
    rows = []
...
<article class="paper">
<div class="eyebrow">Research article · Cross-sectional methodological study · STROBE-aligned</div>
...
<aside class="notice"><strong>Research integrity statement.</strong> Every public observation is synthetic. This article evaluates a reproducible analytical protocol and must not be cited as empirical evidence about Coroico or Bolivia.</aside>

<section class="abstract" id="abstract"><h2>Abstract</h2>
<p><strong>Background:</strong> Household surveys can inform housing and social policy, but public reproducibility may conflict with respondent confidentiality. <strong>Objective:</strong> To specify and evaluate a privacy-preserving, reproducible protocol for analysing housing adequacy and socioeconomic vulnerability. <strong>Methods:</strong> A cross-sectional synthetic population comprised 60 households and 271 members. Referential integrity and disclosure constraints were validated before analysis. Missingness was quantified, weighted descriptive estimands were accompanied by 2,000-resample bootstrap intervals, and conditional associations were estimated using parsimonious logistic regression. <strong>Results:</strong> Weighted adequate housing was {metrics['adequate_housing_weighted_pct']:.1f}% (95% bootstrap interval {metrics['adequate_housing_ci_pct'][0]:.1f}%-{metrics['adequate_housing_ci_pct'][1]:.1f}%). Synthetic median monthly income was BOB {metrics['median_income_bob']:.0f}, with {metrics['income_missing_pct']:.1f}% missing. All exploratory regression intervals crossed the null value, indicating substantial statistical imprecision. <strong>Conclusions:</strong> A privacy-preserving analytical protocol can support computational reproducibility without releasing identifiable microdata. Valid empirical inference would additionally require authorised observations, a documented sampling design and validated measurement instruments.</p>
...
<h3>1.1 Research questions</h3><ol><li>How can adequate housing be operationalised transparently from multiple components?</li><li>Which socioeconomic characteristics are conditionally associated with adequate housing in a small synthetic cross-sectional sample?</li><li>How should missingness and sampling uncertainty be quantified and communicated?</li><li>How can survey analysis remain computationally reproducible without exposing confidential source records?</li></ol></section>

<section id="methods"><h2>2. Methods</h2>
<h3>2.1 Study design and reporting framework</h3><p>This simulation-based methodological study follows the Introduction-Methods-Results-Discussion structure and is mapped to the STROBE checklist for cross-sectional studies [1,2]. It is not a registered observational study and does not estimate population parameters for a real geographic area.</p>
...
<p>Education, log income and household size are standardised. Missing education and income are median-imputed for computational completeness; empirical analysis should use design-aware multiple imputation and sensitivity analysis [5]. No coefficient is interpreted causally.</p></section>
...
<section id="limitations"><h2>5. Limitations</h2><ul><li>Synthetic observations cannot validate or reproduce the original empirical findings.</li><li>The synthetic weights do not represent a real sampling design.</li><li>The housing threshold and vulnerability coefficients require substantive validation.</li><li>Single median imputation understates missing-data uncertainty and is retained solely for computational completeness.</li><li>Binary gender measurement is an artificial simplification.</li><li>Cross-sectional associations cannot identify causal effects.</li></ul></section>
...
<p>A reproducible methodological study of survey estimands, uncertainty, statistical modelling and responsible data communication.</p>
...
<aside class="notice"><strong>Important:</strong> all public observations are synthetic. Results evaluate the analytical protocol and must not be interpreted as estimates for Coroico or Bolivia.</aside>
